...
function DashOrders() {
  ...
  const fetchOrders = async (query = "", start = 0) => {
    setLoading(true);
    try {
      const statusQuery = deliveryStatus !== "all" ? `&dStatus=${deliveryStatus}` : "";
      const res = await fetch(`/api/order/?q=${query.trim()}&start=${start}${statusQuery}`);
        ...
        <>
          <Table hoverable className="mt-4">
            ...
            <Table.Body className="divide-y">
              {orders.map((order) => (
                <Table.Row key={order._id} className="">
                  ...
                  <Table.Cell className=" text-green-500">{order.status}</Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table>
